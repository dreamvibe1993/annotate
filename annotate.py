...
def get_parameter_injected(param_type: str, param_name: str, identation: str) -> str:
	return f"{identation}* @param {{{param_type}}} {param_name} - \n"
...
def find_vertical_annotation(string_to_search_for: str, string_to_search_in: str,
                             search_for: TS_TYPE or METHOD or FUNCTION or LAMBDA) -> str or None:
	...
	annotations: list[str] = re.findall(r'\s*?$\n?\s*?/\*\*\n[^%]+?\*/\n\s*',
	                                    string_to_search_in,
...
def find_horizontal_annotation(string_to_search_for: str, string_to_search_in: str,
...
def find_annotation(string_to_search_for: str, string_to_search_in: str,
                    entity: TS_TYPE or METHOD or FUNCTION or LAMBDA) -> str or None:
	found: str or None = find_vertical_annotation(string_to_search_for, string_to_search_in,
	                                              entity) or find_horizontal_annotation(
...
def replace_modifier_space_with_delimeter(entity: str, content: str) -> Tuple[str, str]:
	modifier_word = ""
	for modifier in modifiers:
		if modifier + " " in entity:
			modifier_word = modifier
	entity_with_delimiter: str = entity.replace(f'{modifier_word} ',
	                                            f"{modifier_word}{delimiter_letters}") if modifier_word else entity
	content_with_delimiter: str = content.replace(entity, entity.replace(f'{modifier_word} ',
	                                                                     f"{modifier_word}{delimiter_letters}")) if modifier_word else content
	return entity_with_delimiter, content_with_delimiter


def replace_delimeter(entity_with_delimiter: str, content_with_delimiter: str) -> Tuple[str, str]:
	entity = entity_with_delimiter
	content = content_with_delimiter
	for modifier in modifiers:
		modifier_word_with_delim = modifier + delimiter_letters
		if modifier_word_with_delim in entity_with_delimiter:
			entity = entity_with_delimiter.replace(entity_with_delimiter,
			                                       entity.replace(modifier_word_with_delim, modifier + " "))
			content = content_with_delimiter.replace(modifier_word_with_delim, modifier + " ")
	return entity, content


def create_annotation(entity: str, content: str, typeof_entity: FUNCTION or METHOD or TS_TYPE or LAMBDA) -> str:
	possible_identation = re.search(rf"\s+?(?={re.escape(entity)})", content)
	identation = re.sub(r"\n", "", possible_identation.group(0)) if possible_identation else ""

	delimited_modified_entities = replace_modifier_space_with_delimeter(entity, content)
	...
	annotated_method_params = annotate_params(method_params, identation)
	new_annotation = f"\n\n{identation}/**\n{identation}* {method_description}\n{annotated_method_params}{identation}*/\n"
	new_annotation = update_annotation(old_annotation, new_annotation, identation)
	original_modified_entities = replace_delimeter(entity, content)
	entity = original_modified_entities[0]
	content = original_modified_entities[1]
	if typeof_entity == METHOD or typeof_entity == FUNCTION:
		content = content.replace(entity, identation + new_annotation + identation + entity)
		...
		if class_constructor:
			if not find_annotation("constructor", ts_class, METHOD):
				...
				if len(class_constructor_params) > 0:
					possible_identation = re.search(rf"\s*(?=constructor)", content)
					identation = possible_identation.group(0) if possible_identation else ""
					constructor_annot_text = f'{identation}/**\n{annotate_params(class_constructor_params, identation)}{identation}*/\n'
					content = content.replace(
						constructor, constructor_annot_text + constructor)
		class_methods: list[str] = re.findall(class_method_pattern, ts_class)
		for class_method in class_methods:
			content = create_annotation(class_method, content, METHOD)
	return content
		...
		for old_annotation_line in re.findall(r"@.+", old_annotation):
			...
			if param_to_replace_to_old_one and line_to_replace_to_old_one:
				new_annotation = re.sub(fr'.+{param_to_replace_to_old_one}.*',
				                        f"{identation}* {line_to_replace_to_old_one}",
				                        new_annotation)
...
def annotate_type(content: str) -> str:
	...
	for whole_ts_type in ts_types:
		...
		possible_identation = re.search(rf"\s*(?={type_declaration})", content)
		identation = possible_identation.group(0) if possible_identation else ""
